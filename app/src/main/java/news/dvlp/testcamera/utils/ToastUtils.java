...
public class ToastUtils {
    ...
    private static Toast sToast, sBlueToast;
    ...
    /*******************************下载专业使用弹框**************/
    // 修改toast显示问题
    private static Toast toastDown;
    private static Timer timer = new Timer();

    // 下载专业使用弹框显示
    public static void showDownToast(CharSequence text) {
        if (toastDown == null) {
            toastDown = Toast.makeText(Utils.getContext(), text, Toast.LENGTH_LONG);
            toastDown.show();
            managerDownToast();
        } else {
            toastDown.setText(text);
        }
    }

    // 控制下载Toast
    private static void managerDownToast() {
     ...
     * @param message
     */
    public static void showBlueToast(Context context, String message) {
        if (isJumpWhenMore) {
            if (sBlueToast != null) {
                sBlueToast.cancel();
                sBlueToast = null;
        ...
        if (sBlueToast == null) {
            //加载Toast布局
            TextView toastRoot = (TextView) LayoutInflater.from(context).inflate(R.layout.layout_toast_view, null);
            //为控件设置属性
            toastRoot.setText(message);
            //Toast的初始化
            sBlueToast = new Toast(context);
            sBlueToast.setDuration(Toast.LENGTH_SHORT);
            sBlueToast.setView(toastRoot);
        } else {
            ((TextView) sBlueToast.getView()).setText(message);
        }
        sBlueToast.show();
    }
}